...
type BadgeTableProps = {
  ...
  onDelete: (badge: BadgeListItemType) => void | Promise<void>;
  onViewDetails: (badge: BadgeListItemType) => void;
    ...
    if (isError) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : safeTranslate("error", "Unable to load badges.");
        ...
        <tr>
          <td colSpan={8} className="p-6">
            <Empty className="border border-dashed">
              <EmptyHeader>
                <EmptyTitle>
                  {safeTranslate("emptyTitle", "No badges yet")}
                </EmptyTitle>
                <EmptyDescription>
                  {safeTranslate(
                    "emptyDescription",
                    "Add a new badge to get started."
                  )}
                </EmptyDescription>
              </EmptyHeader>
              <EmptyContent>
                <Button onClick={onOpenCreate}>
                  ...
                  {safeTranslate("createButton", "Create badge")}
                </Button>
              </EmptyContent>
            </Empty>
          </td>
    ...
    return items.map((badge, index) => {
      ...
      return (
        ...
        >
          ...
          <td className="px-4 py-3">
            <div className="flex flex-col">
              ...
              <span className="text-muted-foreground text-xs">
                {safeTranslate("badgeIdLabel", "Internal ID")}: {badge.id}
              </span>
            </div>
          </td>
          <td className="px-4 py-3">
          ...
          <td className="px-4 py-3">
            <Badge variant="secondary">{badge.badgeCategory}</Badge>
          </td>
          <td className="px-4 py-3">
            {badge.iconUrl ? (
              <div className="flex items-center gap-3">
                <span className="relative h-10 w-10 overflow-hidden rounded bg-muted">
                  <Image
                    src={badge.iconUrl}
                    alt={safeTranslate("columns.iconAlt", "Badge icon image")}
                    fill
                    sizes="40px"
          ...
          <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2">
              ...
              >
                {isDeletePending && deletingId === badge.id ? (
  ...
  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>
            {safeTranslate("tableTitle", "Badge catalogue")}
          </CardTitle>
          <CardDescription>
            {safeTranslate(
              "tableDescription",
              "Review, add or update available badges."
            )}
          </CardDescription>
        </div>
            ...
            <thead>
              <tr className="border-b bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
                ...
                <th className="px-4 py-3 font-medium">
                  {safeTranslate("columns.name", "Badge name")}
                </th>
                <th className="px-4 py-3 font-medium">
                  {safeTranslate("columns.code", "Code")}
                </th>
                <th className="px-4 py-3 font-medium">
                  {safeTranslate("columns.category", "Category")}
                </th>
                <th className="px-4 py-3 font-medium">
      ...
      <CardFooter className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="text-muted-foreground text-sm">
          {pagination.totalItems > 0 ? (
            safeTranslate(
              "pagination.summary",
              `Showing ${pagination.startItem}-${pagination.endItem} of ${pagination.totalItems} badges`,
              {
            ...
            <span>
              {safeTranslate("pagination.empty", "No badges to display")}
            </span>
          )}
        </div>
