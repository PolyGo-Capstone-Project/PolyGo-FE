...
export default function ManageGiftsPage() {
  ...
  const safeTranslate = useCallback<SafeTranslate>(
    (key, fallback, values) => {
      try {
        const namespacedKey = `admin.gifts.${key}`;
  ...
  const [itemToDelete, setItemToDelete] = useState<GiftListItemType | null>(
    null
  );
  const [detailDialogOpen, setDetailDialogOpen] = useState(false);
  const [itemToView, setItemToView] = useState<GiftListItemType | null>(null);
  ...
  const giftsQuery = useGiftsQuery({ params: queryParams });
  const giftQuery = useGiftQuery({
    id: editingId ?? undefined,
    lang,
    enabled: isEditOpen && Boolean(editingId),
  });

  const giftsPayload = giftsQuery.data?.payload;
  const pagination = giftsPayload?.data;
  const rawItems = pagination?.items;
  const items = useMemo<GiftListItemType[]>(() => rawItems ?? [], [rawItems]);
  ...
  const editingGiftFromList = useMemo(
    () => items.find((item) => item.id === editingId) ?? null,
    [items, editingId]
  );

  const createMutation = useCreateGiftMutation(queryParams, {
    onSuccess: (response) => {
  ...
  const updateMutation = useUpdateGiftMutation(queryParams, {
    onSuccess: (response) => {
      showSuccessToast(response.payload?.message, tSuccess);
      setIsEditOpen(false);
      setEditingId(null);
  ...
  const deleteMutation = useDeleteGiftMutation(queryParams, {
    onSuccess: (response) => {
      showSuccessToast(response.payload?.message, tSuccess);
    },
  });
  ...
  const handleViewDetails = (gift: GiftListItemType) => {
    setItemToView(gift);
  ...
  const handleUpdateGift = async (
    ...
    const payload: UpdateGiftBodyType = {
      lang: trimmedLang,
      name: trimmedName,
      description: trimmedDescription,
      price: values.price,
  ...
  const handleDeleteGift = async (gift: GiftListItemType) => {
  ...
  const isListLoading = giftsQuery.isLoading;
  const isListFetching = giftsQuery.isFetching && !giftsQuery.isLoading;
  ...
  const editGift = giftQuery.data?.payload?.data ?? editingGiftFromList;

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-semibold tracking-tight">
          {safeTranslate("title", "Gifts")}
        </h1>
        <p className="text-muted-foreground">
          {safeTranslate(
            "description",
            "Manage gifts available on the PolyGo platform."
          )}
        </p>
      </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {safeTranslate("deleteDialog.title", "Delete gift")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {safeTranslate(
                "deleteDialog.description",
                `Are you sure you want to delete ${itemToDelete?.name ?? "this gift"}? This action cannot be undone.`,
                {
                  name: itemToDelete?.name
                    ? `${itemToDelete.name} (${itemToDelete.price} VND)`
                    : safeTranslate("fallbackName", "this gift"),
                }
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
