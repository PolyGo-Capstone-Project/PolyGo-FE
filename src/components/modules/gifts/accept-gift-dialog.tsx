...
import { useAcceptGiftMutation } from "@/hooks";
import { handleErrorApi } from "@/lib/utils";
import Image from "next/image";

type AcceptGiftDialogProps = {
  open: boolean;
...
export function AcceptGiftDialog({
  open,
  onOpenChange,
  gift,
  locale,
}: AcceptGiftDialogProps) {
  const t = useTranslations("gift.accept");
  const tToast = useTranslations("gift.toast");
  const tError = useTranslations("Error");

  // Accept mutation
  const acceptMutation = useAcceptGiftMutation(
    { lang: locale },
    {
      onSuccess: () => {
        toast.success(tToast("acceptSuccess"));
        onOpenChange(false);
      },
    }
  );
  ...
  const handleAccept = () => {
    acceptMutation.mutate(gift.presentationId, {
      onError: (error) => {
